"""Temporal models for per-pixel TimeFlowCodec."""
...
def fit_linear_time_model(signal: np.ndarray) -> tuple[float, float]:
    """
    Fit s[t] ≈ a + b * t in least-squares sense using closed-form solution.

    Parameters
    ----------
    signal: np.ndarray
        1D array with shape (T,), dtype float.

    Returns
    -------
    (a, b): tuple[float, float]
        Intercept and slope parameters minimizing squared error.
    """
